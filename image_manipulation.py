...
root = CTk()        # to open master window

width = root.winfo_screenwidth()        
height = root.winfo_screenheight()      

root.geometry(f"{width}x{height}+0+0")  # by default size of window
...
def showbuttons():
    ...
    apply_btn = CTkButton(root, text="Apply", width=25, command=apply)
    apply_btn.place(x=1270, y=675)
...
file_image = CTkImage(Image.open("mysortaimages/openfile.png").resize((20,20), Image.LANCZOS))
...
def save():
    saved_img = filedialog.asksaveasfile(defaultextension=".png", filetypes=(("JPG", "*.jpg"), ("PNG", "*.png")))
    if saved_img:
        outputImage.save(saved_img)
...
def resize(x):
    ...
    outputImage = image.resize(sizes[x % len(sizes)])
    resize_btn = CTkButton(root, text="Resize", command=lambda: resize(x + 1)).place(x=3, y=100)
    showimage(outputImage)


def rotate(x):
    global outputImage, rotate_btn
    rotation_values = [90, 180, 270, 0]
    outputImage = image.rotate(rotation_values[x % len(rotation_values)], expand=True, fillcolor=(0, 0, 0))
    rotate_btn = CTkButton(root, text="Rotate", command=lambda: rotate(x + 1)).place(x=3, y=220)
    showimage(outputImage)
...
def color_enhance(x):
    ...
    outputImage = color_enhance.enhance(x)
    showimage(outputImage)
